...
Dura:
    ...
    Loop
    {
        ...
        ImageSearch,,, 10, 90, 260, 120, *30 DuraStrafes\BasicUI\HPBar.bmp
        If (ErrorLevel = 0) {
            f(LeftPID)
            Sleep 3000
            ImageSearch,,, 50, 515, 735, 580, *30 *Trans0x00FF00 DuraStrafes\BasicUI\Dura.bmp
            If (ErrorLevel = 1) {
                Timer := A_TickCount
                Loop
                {
        ...
        if (d2 = "Both") {
            CoordMode, Pixel, Screen
            ImageSearch,,, 810, 90, 1060, 120, *30 DuraStrafes\BasicUI\HPBar.bmp
            If (ErrorLevel = 0) {
                f(RightPID)
                Sleep 3000
                ImageSearch,,, 850, 515, 1535, 580, *30 *Trans0x00FF00 DuraStrafes\BasicUI\Dura.bmp
                If (ErrorLevel = 1) {
                    Timer := A_TickCount
                    Loop
                    {
...
Eat(i, v) {
    ...
    Loop
    {
        ...
        If (ErrorLevel = 1) { 
            If (v = "Inventory") or (v = "Scalar+Inventory") or (v = "Protein+Inventory") {
                ...
                Loop, %var%
                {
                    ImageSearch, x, y, 80, 190, 670, 500, DuraStrafes\BasicUI\x8.bmp
                    If (ErrorLevel = 0) {
                        Click, %x%, %y%, Down
                        Click, %v%, 555, Up
                        v := v + 70
	...
	Loop,
	{
		...
		ImageSearch,,, 125, 135, 140, 150, *30 DuraStrafes\BasicUI\Hunger.bmp
		If (ErrorLevel = 1) { 
			Return "Success"
		}
...
Tag(v = "", i = "") {
    global Shiftlock
    ImageSearch,,, 20, 85, 170, 110, *20 DuraStrafes\BasicUI\combat.bmp
    if (ErrorLevel = 0) {
        SetTimer,, Off
        ...
        if (recordingtype != "Do nothing") {
            ...
            Sleep, 1500
            Sendinput, % sw("o","up")
            SoundPlay, DuraStrafes\Sound\broken.wav
            MouseMove, 575, 120
            Loop, 20
            {
               Send {WheelUp 10}
