...
SetBatchLines -1
SoundPlay, DuraStrafes\Sound\on.wav
Version = 2.1.4
if (A_ScreenDPI != 96) {
...
If !FileExist("DuraStrafes") {
    MsgBox, the data file "DuraStrafes" folder is missing`nExtract file.
    ExitApp
}
global __Index__ := "Basic"
Gui, 2:Font, cBlack, Consolas
Gui, 2:Font, cff0000, Consolas
    ...
    case "Missing" : {
        ...
        If (!UserID) or (!Webhook) {
            ...
            If (GetUrlStatus(Webhook, 10) != 200) {
                ...
                post={"username":"i love Kiyoko's macro","content":"%UserID% hewwo"}
...
Gui, Add, Picture,w27 h30 xm+2 ym+70         gTabDura, DuraStrafes\Icon&Menu\heart.png
...
Gui, Add, Text,ym+110 xm-40             vd13     ,- Buy the durability first.
Gui, Add, Text,ym+150 xm-60             vd14      ,- The macro is configured by Strafes.
...
Treadmill:
    ...
    Loop,
    {
        ...
        Loop
        {
            ImageSearch,,, 20, 120, 260, 140, *20 DuraStrafes\BasicUI\Stamina.bmp
        } Until (ErrorLevel = 0)
        ...
        Loop,
        {
            Loop, Parse, Button, `,
            {
                ImageSearch,,, 200, 240, 600, 300, *50 DuraStrafes\TrainingIcon\%A_LoopField%.bmp
                If (ErrorLevel = 0) {
        ...
        If (T6 != "Strave") {
            ImageSearch,,, 40, 135, 50, 150, *30 DuraStrafes\BasicUI\Hunger.bmp
            If (ErrorLevel = 0) {
                Sleep, 2000
                Click, 410, 345
                Sleep, 1000
...
Weight:
    ...
    Loop,
    {
        ...
        Loop,
        { 
            ImageSearch,x, y, 250, 220, 570, 470, *20 DuraStrafes\TrainingIcon\WeightButton.bmp
            If (ErrorLevel = 0) {
                SetMouseDelay, -1
                MouseMove, x, y, 0
                MouseMove, x+1, y+1, 0
        ...
        If (W4 = "Inventory") or (W4 = "1-0") or (W4 = "Protein+Inventory") {
            If (W4 = "Protein+Inventory") {
                ...
                ImageSearch,,, 0, 145, 70, 170, *10 DuraStrafes\BasicUI\DrinkStatus1.bmp
                If (ErrorLevel = 1) {
                    ...
                    Sleep, 150
                    ImageSearch,,, 65, 525, 750, 585, DuraStrafes\BasicUI\3x2.bmp
                    If (ErrorLevel = 1) { 
    ...
    Loop,
    {
        ...
        If (SP4 != "Strave") {
            If (SP4 = "Scalar+Inventory") {
                i := "3-0"
                Notify("Searching for status")
                ImageSearch,,, 0, 145, 70, 170, *10 DuraStrafes\BasicUI\DrinkStatus2.bmp
                If (ErrorLevel = 1) {
                    Notify("status not found")
        ...
        If (SP2 = "Flow") {
            ImageSearch,,, 390, 115, 430, 180, *10 DuraStrafes\BasicUI\flowstate.bmp
            If (ErrorLevel = 0) {
                Sendinput, % sw("t")
                Sleep 200
            }
...
SS:
    ...
    Loop,
    {
        ...
        ImageSearch,,, 60, 525, 750, 590, *40 DuraStrafes\BasicUI\Speed.bmp
        If (ErrorLevel = 0) {
            ...
            Loop,
            {
                ...
                Sleep 1000
                ImageSearch,,, 65, 525, 750, 585, *40 DuraStrafes\BasicUI\Speed.bmp
...
Dura:
    ...
    Loop
    {
        ...
        ImageSearch,,, 10, 90, 260, 120, *30 SugiPulaDura\BasicUI\HPBar.bmp
        If (ErrorLevel = 0) {
            f(LeftPID)
            Sleep 3000
            ImageSearch,,, 50, 515, 735, 580, *30 *Trans0x00FF00 SugiPulaDura\BasicUI\Dura.bmp
            If (ErrorLevel = 1) {
                Timer := A_TickCount
                Loop
                {
        ...
        CoordMode, Pixel, Window
        f(LeftPID)
        ImageSearch,,, 40, 135, 50, 150, *30 SugiPulaDura\BasicUI\Hunger.bmp
        If (ErrorLevel = 0) {
        ...
        if (d2 = "Both") {
            CoordMode, Pixel, Screen
            ImageSearch,,, 810, 90, 1060, 120, *30 SugiPulaDura\BasicUI\HPBar.bmp
            If (ErrorLevel = 0) {
                f(RightPID)
                Sleep 3000
                ImageSearch,,, 850, 515, 1535, 580, *30 *Trans0x00FF00 SugiPulaDura\BasicUI\Dura.bmp
                If (ErrorLevel = 1) {
                    Timer := A_TickCount
                    Loop
                    {
...
Eat(i, v) {
    ...
    Loop
    {
        ...
        ImageSearch,,, 65, 525, 750, 585, SugiPulaDura\BasicUI\3x2.bmp
        If (ErrorLevel = 1) { 
            If (v = "Inventory") or (v = "Scalar+Inventory") or (v = "Protein+Inventory") {
                ...
                Loop, %var%
                {
                    ImageSearch, x, y, 80, 190, 670, 500, SugiPulaDura\BasicUI\x8.bmp
                    If (ErrorLevel = 0) {
                        Click, %x%, %y%, Down
                        Click, %v%, 555, Up
                        v := v + 70
	...
	Loop,
	{
		...
		ImageSearch,,, 125, 135, 140, 150, *30 SugiPulaDura\BasicUI\Hunger.bmp
		If (ErrorLevel = 1) { 
			Return "Success"
		}
...
Tag(v = "", i = "") {
    ImageSearch,,, 20, 85, 170, 110, *20 SugiPulaDura\BasicUI\combat.bmp
    if (ErrorLevel = 0) {
        SetTimer,, Off
        ...
        if (recordingtype != "Do nothing") {
            ...
            Sleep, 1500
            Sendinput, % sw("o","up")
            SoundPlay, SugiPulaDura\Sound\broken.wav
...
Recorder:
    ...
    Gui, 3:Destroy
    If (!KeyCombo) or (!List) {
        MsgBox,,Kiyoko's Macro, Missing Info
    } else {
    ...
    Loop
    {
        ...
        switch v1 {
            ...
            case "burnfat" : {
                ...
                ImageSearch,,, 0, 145, 70, 170, *10 DuraStrafes\BasicUI\DrinkStatus3.bmp
                If (ErrorLevel = 1) {
                    Notify("status not found")
                    Sendinput, {Backspace}1
                    Sleep, 150
