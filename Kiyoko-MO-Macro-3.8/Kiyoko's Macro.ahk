...
Menu, Tray, Icon, Icons\Menu\KM.png
    MsgBox, 51, Warning Message,1.Put card on slot 0 for Treadmill/Weight/SS `n`2.If you do have the new Roblox UI join the discord and check announcements pinned messages.`n`3.If your macro still doesn't work read #FAQ.`n` `n`Visit Discord?
    IfMsgBox, Yes
    {
...
switch CheckforWebhook() {
    case "Missing" : {
        ...
        Notify("Enter Path")
        InputBox, Path, Enter Path Leave blank for no images ex. C:\Users\frost\Pictures\Roblox,,, 480 , 100 
        If (!UserID) or (!Webhook) {
            ...
            If (GetUrlStatus(Webhook, 10) != 200) {
                ...
                whr := ComObjCreate("WinHttp.WinHttpRequest.5.1")
                post={"username":"KM | Kiyoko's Macro","avatar_url":"https://cdn.discordapp.com/attachments/857916178449563659/1182700889195163648/k-modified1.png?ex=6585a709&is=65733209&hm=ae7f27df2df8deac3540d6cbd27ed2682ee9ce78937032d8aad06f7f4dbfcc8a&", "content":"%UserID% helo"}
                ...
                IniWrite, %Path%, settings.ini, Notifications, Path
    ...
    Loop,
    {
        ...
        Loop,
        {
            ...
            If (A_TickCount - Wait > 10000) {
                Ping("You ran out of money / Not able to click.")Exit(T12)
            } 
        }
        Click, 410, 355, 20
        Notify("Clicked, " T4)
...
CheckforWebhook() {
    ...
    IniRead, Path, settings.ini, Notifications, Path
...
Ping(i) {
    ...
    (
        {
            "username":"KM | Kiyoko's Macro", "avatar_url":"https://cdn.discordapp.com/attachments/857916178449563659/1182700889195163648/k-modified1.png?ex=6585a709&is=65733209&hm=ae7f27df2df8deac3540d6cbd27ed2682ee9ce78937032d8aad06f7f4dbfcc8a&" ,
            "content":"%UserID% %i%"
        }
    )
    whr.Open("POST", Webhook, False),whr.SetRequestHeader("Content-Type", "application/json"),whr.Send(post)
...
ImageSend:
    ; read the image directory path from the .ini file
    IniRead, Path, settings.ini, Notifications, Path
    IniRead, Webhook, settings.ini, Notifications, Webhook
    ...
    If (Path = "")
    {
        Ping("Image directory path is not specified in the settings.ini file. Macro stopped.")
        MsgBox, Image directory path is not specified in the settings.ini file.
        ExitApp
    }
    ...
    Sleep, 2000 ; if you want you could adjust the sleep but watch out

    ; Delete the latest image file
