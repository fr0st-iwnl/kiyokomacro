    ...
    case "Missing" : {
        ...
        If (!UserID) or (!Webhook) {
            ...
            If (GetUrlStatus(Webhook, 10) != 200) {
                ...
                post={"username":"KM | Kiyoko's Macro","avatar_url":"https://cdn.discordapp.com/attachments/805554377745235974/1146610967623520347/kiyokomacro2.png", "content":"%UserID% helo"}
...
Gui, Add, DDL, w150 ym+110 xm    vT6            ,Inventory|1-0|Starve
...
Gui, Add, Text, ym+200 xm-210    vT17         ,- Put card on 0
Gui, Add, Button, w100 x356 y250      vT18 gTreadmill,Done
Gui, Margin, 80, 50
Gui, Add, Text,ym xm             vW1            ,Focused Level
Gui, Add, DDL, w150 ym+20 xm     vW2            ,6|5|4|3|2|1|All
Gui, Add, Text,ym+45 xm          vW3            ,Eating Method 
Gui, Add, DDL, w150 ym+65 xm     vW4            ,1-0|Protein+Inventory|Starve
...
Gui, Add, Button, w100 x356 y250      vSP14   gSP    ,Done
Gui, Add, Text,ym+190 xm-230             vSP15      ,- Equip combat when u start the macro !
; Gui, Add, Text,ym+182 xm-230             vSP16      ,- If ur stamina freezes pick a better stamina regen that fits urs.
; Gui, Add, Text,ym+200 xm-230             vSP17      ,- If not disable stamina freeze from settings.ini
Gui, Margin, 80, 50
...
Gui, Add, Button, w100 x326 y150      vSS15   gSSOLD2    ,F1 | OLD SS
Gui, Margin, 80, 50
...
Gui, Margin, 80, 50
Gui, Add, Text, ym+-15 xm vm3 , Basic Run Macro
Gui, Add, Button, ym20 xm vm4 w70 grunmacro, Start
Notify("Loaded Ui")
    ...
    Loop,
    {
        ...
        Loop,
        {
            ...
            If (A_TickCount - Wait > 10000) {
                Ping("You ran out of money.")Exit(T12)
            } 
        }
        Click, 410, 355, 20
        Notify("Clicked, " T4)
        ...
        Loop,
        {
            Loop, Parse, Button, `,
            {
                ImageSearch,,, 200, 240, 600, 300, *50 Icons\TrainingIcon\%A_LoopField%.bmp
                If (ErrorLevel = 0) {
...
Weight:
    ...
    Loop,
    {
        ...
        Loop,
        {
            ImageSearch,x, y, 250, 220, 570, 470, *20 Icons\TrainingIcon\WeightButton.bmp
            If (ErrorLevel = 0) {
                MouseMove, x, y, 0
                MouseMove, x+1, y+1, 0
                Click, 10
    ...
    Loop,
    {
        ...
        If (SP4 != "Starve") {
            If (SP4 = "Scalar+Inventory") {
                i := "3-0"
                Notify("Searching for status...")
                ImageSearch,,, 0, 145, 70, 170, *10 Icons\BasicUI\DrinkStatus2.bmp
                If (ErrorLevel = 1) {
        ...
        If (SP2 = "Flow") {
            ImageSearch,,, 390, 115, 430, 180, *10 Icons\BasicUI\flowstate.bmp
            If (ErrorLevel = 0) {
                Sendinput, % sw("t")
                Sleep 200
            }
...
muscle:
    ...
    Gui, Add, DDL, w150 ym+65 xm vv2,Inventory|2-0|Starve
...
loop,
{
    ...
    {
        Send 1r
        Notify("Macro is charging Rythm.")
    }
    else
    {
        Send 1
    ...
    Loop, 4
    {
        ...
        {
            Send {Click}
            Notify("Macro is doing a M1")
            Sleep 1050
        }
        Sleep 100
        Send {Click, Right}
        Notify("Macro is doing a M2")
        Sleep 1050
    }
    Send {Click}
    Sleep 1100
...
Ping(i) {
    ...
    (
        {
            "username":"KM | Kiyoko's Macro", "avatar_url":"https://cdn.discordapp.com/attachments/805554377745235974/1146610967623520347/kiyokomacro2.png" ,
            "content":"%UserID% %i%"
        }
    )
    whr.Open("POST", Webhook, False),whr.SetRequestHeader("Content-Type", "application/json"),whr.Send(post)
    ...
    Loop
    {
        ...
        If (ErrorLevel = 1) { 
            If (v = "Inventory") or (v = "Scalar+Inventory") or (v = "Protein+Inventory") {
                ...
                Loop, %var%
                {
                    ImageSearch, x, y, 80, 190, 670, 500, Icons\BasicUI\x8.bmp
                    If (ErrorLevel = 0) {
                        Click, %x%, %y%, Down
                        Click, %v%, 555, Up
                        v := v + 70
...
runmacro:
    ...
    gui, add, ddl, ym vv1,run|starve|starve+knock|burnfat
    if (i != "ERROR") or (!i) {
